# GPU 部品の後入れ Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** NVIDIA の GPU がある人だけが、アプリの中から GPU 部品（cuBLAS / cuDNN）を PyPI の公式 wheel から導入・削除できるようにし、CUDA 版のビルドをやめる。

**Architecture:** Python の `utils` が部品の置き場所（`<データの置き場所>\cuda`）・版の印・削除の予約を扱い、起動時に一致する部品だけを DLL 検索パスに載せる。新しいモジュール `models/cuda_pack.py` が状態の判定と取得・検証・展開を行い、Controller がエンドポイント・進み具合・導入後の GPU への切り替えを受け持つ。画面は状態を受け取って設定欄と初回の問いかけを出し、再起動は Rust の新しいコマンド `app_restart` が行う（準備済みの更新があれば更新役に任せる）。
...
- 部品の版の印 `cu12.8-cudnn9.7`。取得する wheel（URL・大きさ・SHA-256 はこの値をそのまま使う）:
...
1. **部品の版の印を上げた新しい版のアプリを入れたとき（古い部品が残っている）** — 古い DLL を読み込まずに `not_installed` になり、導入し直しで `bin\` を置き換えられること（読み込み済みで使用中だと置き換えに失敗するため）。→ Task 1 の `test_mismatched_pack_is_not_registered`。
2. **2 つ目の wheel の途中で通信が切れたとき** — 何も入らず、前から入っていた部品があればそのまま残り、画面は `not_installed` に戻ってエラーを出すこと。→ Task 1 の `test_failure_keeps_an_existing_installation`、Task 2 の `test_failed_download_reports_error_and_status`。
3. **初回の問いかけと設定の両方で「導入する」を押す（二重押し）** — ダウンロードは 1 本だけ走ること。→ Task 2 の `test_second_download_request_is_ignored`。
4. **部品を導入して再起動したのに GPU が一覧に出ない（ウイルス対策が DLL を止めた、ドライバーが変わった等）** — 落ちずに CPU のまま動き、エラーを 1 回だけ出し、次の起動で繰り返さないこと。→ Task 2 の `test_gpu_missing_after_install_reports_once`。
5. **DLL を読み込んだまま「削除」を押す** — その場では消さずに予約だけし、次の起動で DLL を読み込む前に消すこと。→ Task 1 の `test_pending_removal_deletes_before_registration`。

---

### Task 1: 部品の置き場所・状態・取得（Python の中心部分）
...
3. `_registerCudaLibraries` の docstring から CUDA 版ビルドと `spec/backend_cuda.spec` の話を消し、「手元の venv に `nvidia-*` が入っていればその bin も載せる。アプリの GPU 部品は externalCudaLibraryDir()」に書き換える。本体を次にする:
...
5. `_getCudaDeviceNames` の上のコメントの「CUDA版ビルドだけが nvidia-cublas-cu12 でこれを持つ」を「GPU 部品 (data\\cuda\\bin) を導入したときだけこれを持つ」に直す。
...
git commit -m "feat(cuda): GPU 部品の置き場所・状態・取得と展開、削除の予約を作る"
```
...
git commit -m "feat(cuda): GPU 部品のエンドポイントと、導入後の起動で GPU に切り替える処理を足す"
```
...
`_components/index.js` に `export { CudaPack } from "./cuda_pack/CudaPack";` を足す（ほかの部品と同じ書き方に合わせる）。
...
```yaml
        cuda_pack:
            label: "GPU 部品"
            desc: "NVIDIA の GPU で翻訳と文字起こしを速くする部品です（ダウンロード約 1.3 GB、展開後 1.8 GB）。導入したあと、再起動すると使えます。"
...
```yaml
cuda_pack_prompt:
    title: "GPU で翻訳と文字起こしを速くする部品を導入しますか？"
    desc: "NVIDIA の GPU が見つかりました。部品を導入すると、翻訳と文字起こしを GPU で速く動かせます（ダウンロード約 1.3 GB、展開後 1.8 GB）。あとからでも、設定の翻訳・文字起こしのデバイス欄で導入できます。"
    install_button: "導入する"
    later_button: "今はしない"
```
...
```yaml
    failed_download_cuda_pack: "GPU 部品を導入できませんでした。通信と空き容量（約 3.1 GB 必要）を確かめて、もう一度試してください。"
    cuda_pack_not_loaded: "GPU 部品を読み込めませんでした。CPU で動きます。"
```
...
```yaml
        cuda_pack:
            label: "GPU parts"
            desc: "Parts that make translation and transcription faster on NVIDIA GPUs (about 1.3 GB download, 1.8 GB on disk). They take effect after a restart."
            ...
            remove_pending: "They will be removed at the next start."
            restart_button: "Restart"
```

```yaml
cuda_pack_prompt:
    title: "Install the parts that make translation and transcription faster on your GPU?"
    desc: "An NVIDIA GPU was found. With these parts, translation and transcription run faster on the GPU (about 1.3 GB download, 1.8 GB on disk). You can also install them later from the device settings for translation and transcription."
    install_button: "Install"
    later_button: "Not now"
```

```yaml
    failed_download_cuda_pack: "Could not install the GPU parts. Check your connection and free disk space (about 3.1 GB needed), then try again."
    cuda_pack_not_loaded: "Could not load the GPU parts. The app runs on the CPU."
```
...
git commit -m "feat(ui): GPU 部品の設定欄と初回の問いかけを足し、再起動で使えるようにする"
```
...
**Interfaces:**
- Consumes: なし
- Produces: CUDA 版のビルドが無くなった状態（GPU は Task 1〜4 の部品で使う）
...
- `requirements.txt` 先頭の NOTE の `requirements_cuda.txt` に触れている文を「GPU は アプリから導入する GPU 部品（models/cuda_pack.py）で使う」に書き換える。
...
- `docs/readme_build.md`: CUDA 版の節（Python 環境のセットアップの `.venv_cuda` の説明、「ビルドの種類」の「CUDA版」、「CUDA版の開発ビルド」、高速開発ビルドの `dev-cuda-fast`、Python バックエンドのビルドの「CUDA版」、PyInstaller の環境変数の `build-python-cuda`、ディレクトリ構成の `build_cuda.bat` / `backend_cuda.spec` / `requirements_cuda.txt`、「CUDA版が動作しない」、スペックファイルの `backend_cuda.spec`）を消すか、「GPU はアプリの設定から GPU 部品を導入して使う（ダウンロード約 1.3 GB、`<データの置き場所>\cuda\bin`）。開発中も同じで、`.venv_cuda` は要らない」という説明に置き換える。
...
git commit -m "build: CUDA 版のビルドをやめ、GPU はアプリから導入する部品で使う"
```
...
**Files:**
- Modify: `docs/perf/README.md`（末尾に「追記（GPU 部品の後入れ）」）
- Modify: `docs/superpowers/specs/2026-09-24-cuda-pack-design.md`（確かめた結果で変わった点があれば）
- Local only（コミットしない）: `.claude/skills/run-vrct/SKILL.md` に GPU 部品の確認手順を足す
...
Velopack で入れた版を手元の更新元で導入して起動し、初回の問いかけ（「GPU で翻訳と文字起こしを速くする部品を導入しますか？」）が出ることを確かめる。「導入する」を押し、本物の PyPI から取得が進み、設定の GPU 部品欄に進み具合が出ることを確かめる。かかった時間と `data\cuda\bin` の大きさを記録する。もう一度起動して、問いかけが 2 回目は出ないことを確かめる。

- [ ] **Step 3: 再起動して GPU を使う**

「再起動して GPU を使う」を押し、起動し直したあとで次を確かめる: 翻訳と文字起こしのデバイスが GPU（計算の種類は自動）、GPU 部品欄が「削除」、サイドカーのプロセスが `cublas64_12.dll` と `cudnn*.dll` を読み込んでいる（`Get-Process -Id <sidecar> | Select -Expand Modules`）、翻訳 1 通と文字起こし（マイクが使えなければ翻訳だけ）が GPU で動く。CPU との翻訳の速さを記録する。

- [ ] **Step 4: 削除**

「削除」→「次の起動で削除します」→「再起動」で起動し直し、`data\cuda` が消えてデバイスが CPU に戻り、GPU 部品欄が「導入する」になることを確かめる。
...
アプリを削除し、`%LocalAppData%\VRCT-0` が部品ごと消えること、元の VRCT が変わっていないこと、自分が起動したプロセスが残っていないことを確かめる。`docs/perf/README.md` に結果（取得時間・大きさ・GPU と CPU の速さ・確かめられなかった項目）を書き、spec と違う振る舞いがあれば spec を直す。

```powershell
git add docs/perf/README.md docs/superpowers/specs/2026-09-24-cuda-pack-design.md
git commit -m "docs: GPU 部品の後入れの実機確認の結果を記録"
```
