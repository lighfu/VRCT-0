...
結果（2026-09-24）は `docs/perf/README.md` の「追記（GPU 高速化パックの後入れ）」。1・2・4・5 は期待どおり（取得 1.28 GB は 67〜126 秒、展開後 1,766 MiB、翻訳は GPU で約 4.3 倍）。3 は §4 の問題で DLL が 2 つ残った。文字起こしは Whisper のモデルを GPU に作るところまでで、声を入れた推論は確かめていない。
