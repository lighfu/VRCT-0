...
- **設定**: 翻訳と文字起こしの「デバイス」欄の下に「GPU 部品」欄を置く（中身は 1 つで両方に出す）。文字起こしのデバイス欄は音声認識エンジンが Whisper のときだけ出るので、文字起こし側の GPU 部品欄も Whisper のときだけ出る（既定のエンジンは Google）。状態に応じて:
  - GPU が無い → 欄を出さない
...
- **導入後**: 「再起動して GPU を使う」を出す。再起動後、翻訳と文字起こしのデバイスを自動で GPU（計算の種類は「自動」）にする（部品を導入した直後の起動の 1 回だけ）。音声認識エンジンは切り替えないので、文字起こしが GPU で動くのは Whisper を選んだとき。
- **削除**: 「削除」で「次の起動で削除します」と出して再起動を促す（DLL は使用中で消せないため）。次の起動で DLL を読む前に消し、デバイスは CPU に戻る。ただし今は、再起動の前のサイドカーが残る問題（§4）のため、GPU を使っていたときは `cublas64_12.dll` と `cublasLt64_12.dll` が消えずに残る。
...
**実機で見つかった問題（未修正、2026-09-24）**: 準備済みの更新が無いときの再起動では、前のサイドカーが終わらずに残る。`app_restart` は `async` の付かないコマンドなのでメインスレッドで動き、Tauri 2.5.1 の `restart()` はメインスレッドから呼ばれると `RunEvent::Exit` を出さずにプロセスを起動し直す。tauri-plugin-shell はサイドカーを `RunEvent::Exit` でだけ止め、サイドカーは `/run/shutdown` では終わらない（watchdog も止まる）ので、誰も止めない。GPU を使っていたサイドカーが `data\cuda\bin` の cuBLAS を読んだまま残るため、削除で 2 つの DLL が消えず、そのあとの導入は落とし終えてから `PermissionError` で失敗する。準備済みの更新があるときの道（`app.exit(0)`）では起きない。詳しくは `docs/perf/README.md` の「追記（GPU 部品の後入れ）」。
...
結果（2026-09-24）は `docs/perf/README.md` の「追記（GPU 部品の後入れ）」。1・2・4・5 は期待どおり（取得 1.28 GB は 67〜126 秒、展開後 1,766 MiB、翻訳は GPU で約 4.3 倍）。3 は §4 の問題で DLL が 2 つ残った。文字起こしは Whisper のモデルを GPU に作るところまでで、声を入れた推論は確かめていない。
