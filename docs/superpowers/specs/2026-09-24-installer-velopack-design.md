...
- 入れてある上から Setup を実行したとき（修復・更新・ダウングレード）は、Setup が導入先を丸ごと
  `%LocalAppData%\VRCT-0.<英数字16文字>` に退避して入れ直し、終わると退避したフォルダを消す。
  何もしないと `data\` も一緒に消える（2026-09-24 に 3 通りとも実機で確認）ので、導入のフックで退避したフォルダから `data\` を戻す（§2）。
- VRCT-0 はこのフォルダの外に何も書かない。WebView2 のデータ（今は `%LOCALAPPDATA%\com.vrct.app`）も `data\webview` に移す。
  サイドカーが使うライブラリの既定の置き場所も `data\` に向ける: Hugging Face のキャッシュ（huggingface_hub と hf_xet。
  既定は `%USERPROFILE%\.cache\huggingface`）は `HF_HOME` で `data\huggingface` に、NVIDIA のドライバーが CUDA を使うときに作る
  JIT のキャッシュ（既定は `%APPDATA%\NVIDIA\ComputeCache`）は `CUDA_CACHE_PATH` で `data\nvidia\ComputeCache` に。
  2026-09-24 に空のユーザープロフィールを真似てサイドカーを動かし、プロフィールの下に書かれたのは ComputeCache だけだった
  （配るサイドカーは hf_xet の配布情報を含まないので、huggingface_hub は hf_xet を使わず、今は `.cache\huggingface` にも書かない）。
  Tauri のプラグインのうち `%LOCALAPPDATA%\com.lighfu.vrct0` に書く tauri-plugin-http（クッキーの保存）と tauri-plugin-fs は、
  画面から使っていないので入れない。
...
- 入れてある上から実行すると、Setup は版に応じて「修復」（同じ版）・「更新」（新しい版）・「ダウングレード」（古い版）を尋ね、
  導入先を退避して入れ直す（§1）。退避したフォルダを消す前に導入のフック（`--veloapp-install`）が新しい `VRCT-0.exe` で呼ばれるので、
  そこで退避したフォルダの `data\` を導入先へ名前の変更で戻す（同じドライブなので一瞬。フックの制限の 30 秒に収まる）。
  退避したフォルダが複数あれば `data\` が新しいものを使い、導入先の `data\` に中身があれば触らない。
  残る穴: フックのあとで Setup 自体が失敗すると（アンインストールの登録やアプリの起動の失敗）、Setup は新しい導入先ごと消して
  退避したフォルダを戻すので、戻した `data\` も消える。
...
- 削除の直前のフックでは、Setup が消し損ねた退避フォルダ（`%LocalAppData%\VRCT-0.<英数字16文字>`。古い `data\` が入っていることがある）も消す。
...
3. 落とし終えたら「今すぐ再起動して更新」と「終了するときに更新」の説明を出す。何も押さなければ、次にアプリを閉じたときに黙って入れ替える（再起動はしない）。アプリが落ちて閉じる処理を通らなかった場合は、次の起動時に Velopack が自動で入れ替える（Velopack の既定の動き。計画の段階で `VelopackApp::run()` の実装を読んで確認し、2026-09-24 に実機でも確認。入れ替えたあと新しい版で起動し直す）。
   ただしこれは版が上がる更新だけで、版を下げる更新（ベータ版→安定版）は、次の起動で `VelopackApp::run()` が今の版より古いパッケージとして消すので入れ替わらず、次の確認でまた通知する。
...
- 確認は 30 秒で諦める（Velopack の GitHub のソースには通信の時間切れが無く、スリープからの復帰などで止まると終わらないため）。
  起動時の確認なら何も出さずにログだけ、「更新を確認」を押したときは失敗（`timeout`）と表示する。
  ダウンロードが途中で止まった場合は、今はアプリを再起動するまで「ダウンロード中」のまま（Velopack のロックを持ったまま止まるので、
  やり直すには再起動が要る。最初の公開リリースの前に手当てする）。
...
1. 手元の `vpk pack` で版を 2 つ作り（例: `3.5.1-beta.1` と `3.5.1-beta.2`）、ローカルのフォルダを更新元にして次を確かめる:
   ...
   - 入れてある上から Setup を実行しても（同じ版・新しい版・古い版）、設定とモデルが残ること
   - 2026-09-24 に確かめた。結果は `docs/perf/README.md` の「追記（Velopack のインストーラー）」。
     このとき見つかった導入先の外への書き込み（tauri-plugin-http の `%LOCALAPPDATA%\com.lighfu.vrct0\.cookies`）と、
     上から Setup を実行すると `data\` が消えることは直し、同じ節の「修正後の再確認」で確かめ直した。
